...
public class EntityManager {
	 ...
	 * @param pageable   page parameter, or null, if everything needs to be
	 *                   returned.
		...
		if (pageable != null) {
			if (where != null) {
				return createSelectImpl(
						selectFrom.limitOffset(pageable.getPageSize(), pageable.getOffset()).where(where), entityType,
						pageable.getSort());
			} else {
				return createSelectImpl(selectFrom.limitOffset(pageable.getPageSize(), pageable.getOffset()),
						entityType, pageable.getSort());
			}
		} else {
			if (where != null) {
	...
	public Mono<Long> updateLinkTable(LinkTable table, Object entityId, Stream<?> referencedIds) {
		return deleteFromLinkTable(table, entityId)
				.then(Flux.fromStream(referencedIds).flatMap((Object referenceId) -> {
					StatementMapper.InsertSpec insert = r2dbcEntityTemplate.getDataAccessStrategy().getStatementMapper()
							.createInsert(table.tableName).withColumn(table.idColumn, Parameter.from(entityId))
							.withColumn(table.referenceColumn, Parameter.from(referenceId));

					return r2dbcEntityTemplate.getDatabaseClient().sql(statementMapper.getMappedObject(insert)).fetch()
							.rowsUpdated();
				}).collectList().map((List<Long> updates) -> updates.stream().reduce(Long::sum).orElse(0l)));
	}
	...
	public Mono<Void> deleteFromLinkTable(LinkTable table, Object entityId) {
		...
		StatementMapper.DeleteSpec deleteSpec = r2dbcEntityTemplate.getDataAccessStrategy().getStatementMapper()
				.createDelete(table.tableName).withCriteria(Criteria.from(Criteria.where(table.idColumn).is(entityId)));
	...
	/**
	 * Creates an OrderByField instance for sorting a query by the specified
	 * property.
	 *
	 * @param propertyName The full property name in the format
	...
	private static OrderByField createOrderByField(String propertyName) {
		...
		return OrderByField.from(
				// Create a column with the given name and alias it with the table name and
				// column name
