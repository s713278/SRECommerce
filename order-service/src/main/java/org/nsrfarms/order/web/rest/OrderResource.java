package org.nsrfarms.order.web.rest;
...
import org.nsrfarms.order.service.impl.PaginationUtil;
import org.nsrfarms.order.web.rest.error.BadRequestAlertException;
import org.nsrfarms.order.web.rest.error.HeaderUtil;
...
/**
 * REST controller for managing {@link com.mycompany.gateway.domain.Order}.
 */
@RestController
...
public class OrderResource {
	...
	public Mono<ResponseEntity<OrderDTO>> createOrder(@Valid @RequestBody OrderDTO orderDTO) throws URISyntaxException {
		...
		return orderService.save(orderDTO).map(result -> {
			try {
				return ResponseEntity.created(new URI("/api/orders/" + result.getId())).headers(HeaderUtil
	 ...
	 */
	@PutMapping("/{id}")
	public Mono<ResponseEntity<OrderDTO>> updateOrder(@PathVariable(value = "id", required = false) final Long id,
			@Valid @RequestBody OrderDTO orderDTO) throws URISyntaxException {
		...
		return orderRepository.existsById(id).flatMap(exists -> {
			...
			return orderService.update(orderDTO)
					...
					.map(result -> ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true,
	...
	public Mono<ResponseEntity<OrderDTO>> partialUpdateOrder(
			@PathVariable(value = "id", required = false) final Long id, @NotNull @RequestBody OrderDTO orderDTO)
		...
		return orderRepository.existsById(id).flatMap(exists -> {
			...
			return result.switchIfEmpty(Mono.error(new ResponseStatusException(HttpStatus.NOT_FOUND)))
					.map(res -> ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true,
	...
	public Mono<ResponseEntity<List<OrderDTO>>> getAllOrders(
			@org.springdoc.core.annotations.ParameterObject Pageable pageable, ServerHttpRequest request) {
	...
	public Mono<ResponseEntity<OrderDTO>> getOrder(@PathVariable("id") Long id) {
		...
		return org.nsrfarms.order.web.rest.error.ResponseUtil.wrapOrNotFound(orderDTO);
	}
