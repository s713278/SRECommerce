...
import org.nsrfarms.order.web.rest.error.ProblemDetailWithCause.ProblemDetailWithCauseBuilder;
...
public class ExceptionTranslator extends ResponseEntityExceptionHandler implements ExceptionTranslation {
	...
	protected Mono<ResponseEntity<Object>> handleExceptionInternal(Exception ex, @Nullable Object body,
			HttpHeaders headers, HttpStatusCode statusCode, ServerWebExchange request) {
		body = body == null ? wrapAndCustomizeProblem((Throwable) ex, (ServerWebExchange) request) : body;
		if (request.getResponse().isCommitted()) {
			return Mono.error(ex);
		}
		return Mono.just(new ResponseEntity<>(body, updateContentType(headers),
	...
	private String getCustomizedErrorDetails(Throwable err) {
		...
		if (activeProfiles.contains("prod")) {
			...
			// if (containsPackageName(err.getMessage())) return "Unexpected runtime
			// exception";
		}
	...
	private HttpHeaders buildHeaders(Throwable err) {
		return err instanceof BadRequestAlertException badRequestAlertException
				? org.nsrfarms.order.web.rest.error.HeaderUtil.createFailureAlert(applicationName, true,
						badRequestAlertException.getEntityName(), badRequestAlertException.getErrorKey(),
	...
	private boolean containsPackageName(String message) {
		...
		/*
		 * return StringUtils.containsAny( message, "org.", "java.", "net.", "jakarta.",
		 * "javax.", "com.", "io.", "de.", "com.mycompany.gateway" );
		 */
		return true;
	}
}
